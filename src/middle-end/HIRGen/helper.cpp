...
HIRNode *HIRGenerator::emit_call(ASTNode_t *node) {
  ...
  call_node->call.args = new std::vector<HIRNode *>(); // Make sure this is a
                                                       // brand new clean vector
  ...
  for (ASTNode_t *curr = node->call.args; curr;) {
    ...
    // This will safely create the "__deref_temp" and store it in
    // side_effect_buffer
    HIRNode *lowered_arg = generate(arg_expr);

    // 🎯 FORCE: Only the returned placeholder temp variable should ever be
    // added to the call parameters!
    if (lowered_arg) {
