...
Type_t *binop(ASTNode_t *n, Type_t *type) {
  ...
  if (n->bin.left->kind == AST_NUM &&
      (!n->bin.left->type || n->bin.left->type->base == UNKNOWN)) {
    if (is_numeric(rt->base)) {
      n->bin.left->type = rt;
      lt = rt;
    }
  } else if (n->bin.right->kind == AST_NUM &&
             n->bin.right->type->base == UNKNOWN) {
    if (is_numeric(lt->base)) {
      n->bin.right->type = lt;
      rt = lt;
    }
  }
  ...
  if (lt && lt->base == STRINGS || rt && rt->base == STRINGS) {
    ...
    bool is_valid_add =
        (n->bin.op == OP_ADD) && (lt->base == STRINGS && rt->base == STRINGS);
  ...
  case OP_NEQ:
    if (!is_numeric(lt->base) || !is_numeric(rt->base)) {
      // If they aren't numeric, allow EQ/NEQ only if they are the exact same
      // type
      if ((n->bin.op == OP_EQ || n->bin.op == OP_NEQ) &&
          types_are_equal(lt, rt)) {
  ...
  default:
    ...
    n->type = n->type->base == promted_t ? n->type
              : promted_t == UNKNOWN     ? NULL
                                         : make_type(promted_t, NULL);
    return n->type;
  }
}
