...
#include "semantic/TypeResolver.hpp" // New include
...
const char *safe_get_target_name(ASTNode_t *lhs) {
  ...
  return lhs && lhs->kind == AST_UNOP && lhs->unop.op == OP_DEREF ? "ptr_target"
                                                                  : "unknown";
}
...
void process_declaration(ASTNode_t *n, Type_t *&lhs_t, Type_t *rhs_t) {
  ...
  if (lhs_t->base != UNKNOWN && is_numeric(lhs_t->base) && rhs &&
      rhs->kind == AST_NUM) {
  ...
  if (!SV_semantic_declare(var_name, &n->isglobal, lhs_t, n,
                           n->assign.is_mutable)) {
    panic(n->loc, SEM_VAR_REDECL, var_name);
  }
}

// 1. FIXED RESPONSIBILITY: Safely look up exactly what container type LHS is
// targeting
void resolve_target_type(ASTNode_t *n, Type_t *&type) {
  ...
  if (lhs->kind == AST_VAR) {
    ...
    if (name && name[0] == '@')
      n->isglobal = lhs->isglobal = true;

    if (n->assign.is_declaration) {
      if (!n->type)
        n->type = make_type(UNKNOWN, nullptr);
      type = n->type;
    } else {
    ...
    Type_t *resolved_lhs_type =
        SV::TypeChecker::check_expr(lhs); // Use TypeChecker's check_expr
    if (resolved_lhs_type) {
      type = resolved_lhs_type; // This will correctly resolve to STRINGS
    } else {
      panic(n->loc, SEM_ASSIGN_TYPE_MISMATCH,
            "Dereference target must resolve to a valid type");
    }
  }

  else if (lhs->kind == AST_INDEX) {
    SV::TypeChecker::handle_idx_assign(
        n, lhs, type); // Use TypeChecker's handle_idx_assign
  } else {
...
bool verify_expression_path_is_mutable(ASTNode_t *n) {
  ...
  if (n->kind == AST_VAR) {
    ...
    // (Using SV_semantic_lookup_symbol_record for local scope lookup)
    SemanticSymbolRecord *sym =
        n->isglobal ? semantic_symbol_table::semantic_find_global_symbol(n->var)
                    : SV::semantic_symbol_table::semantic_find_symbol(n->var);
    if (sym) {
  ...
  if (n->kind == AST_UNOP && n->unop.op == OP_DEREF) {
    ...
    if (n->unop.operand && n->unop.operand->type) {
      if (n->unop.operand->type->base == PTR) {
        ...
        // sets a flag like 'is_mut_ptr' or check if the container allows
        // writing.
...
Type_t *assign(ASTNode_t *n, Type_t *type) {
  ...
  Type_t *rhs_t =
      check_expr(n->assign.rhs, lhs_t); // Use TypeChecker's check_expr
  ...
  if (n->assign.is_declaration) {
    ...
    //   exitcode_t ac = SV_semantic_assign_check(target_name,
    //   n->assign.lhs->isglobal,
    //                                           rhs_t->base,
    ...
    if (n->assign.lhs->kind == AST_INDEX) {
      ...
      while (curr_idx != nullptr) {
        ...
        if (!itype || (!is_integer(itype->base))) { // Changed to is_integer for
                                                    // broader integer support
